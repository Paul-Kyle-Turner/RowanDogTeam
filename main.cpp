...
int main(void) {
	...
	int carCount = 0;
	int carCount1 = 0;
	capVideo.open("rtsp://tech:tech@72.82.144.138:556/cam/realmonitor?channel=8&subtype=1");  //@CW opens the RTSP feed as the feed, i'd like to eventually not make this hard coded.
	...
	int intHorizontalLinePosition = 100;
	...
	int intVerticalLinePosition = 250; //@CW - position of the line cars going right to left need to hit to increment carCount1. 
	
	//@CW positions for the visual lines on the screen. crossingLine was originally a horizontal line, so I just switched the x and y values.
	crossingLine[0].y = 0;
	...
	crossingLine1[0].y = 0;
	crossingLine1[0].x = 250;

	crossingLine1[1].y = imgFrame1.cols - 1;
	crossingLine1[1].x = 250;
...
bool checkIfBlobsCrossedTheLineFromLeft(std::vector<Blob> &blobs, int &intHorizontalLinePosition, int &carCount) //@CW checks if blobs cross the line from right side, needed to change the greater than and less than signs
{ 
	bool blnAtLeastOneBlobCrossedTheLine = false;
...
void drawCarCountOnImage1(int &carCount, cv::Mat &imgFrame2Copy) //@CW draws count on image, needed to create a seperate function to change where the count is displayed
{
